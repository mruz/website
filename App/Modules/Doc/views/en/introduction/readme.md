...
Simple and fast PHP framework delivered as a C-extension.
...
*please do not send files from the `build/` directory (they are generated automatically), just those from the `ice/`.
...
### Compilation*
* Compile on Linux/Unix/Mac. There are two ways to compile the Ice framework from source:
 * compile from the `build/` directory and C-files - recommended for beginners and for production machines; C-files are generated by the Ice Team after a merge once tests pass
 * build with [zephir](https://github.com/ice/zephir) (generate, compile and install the extension) - recommended for developers and for testing the next release; this way you generate C-files from Zep-files, so you get the newest features/fixes, but it could be unstable; after generating with zephir, the files will be located in the `ext/` directory
...
###### By default `./vendor/bin/zephir` runs zephir. To build Ice, you must run zephir from inside the `framework/` directory:
```sh
cd framework/
./vendor/bin/zephir build
```

*if you don't have zephir, you need to compile it.

##### Compile zephir
###### You can install zephir using composer. If you already have Ice and are inside the `framework/` directory:
```sh
composer update
```
...
#### Requirements
You can build from C-files or build with zephir.
